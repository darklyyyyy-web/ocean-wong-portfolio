...
function extractImageSortName(image = {}) {
  if (image.fileName) {
    return image.fileName;
  }

  if (image.storage_path) {
    const storageName = image.storage_path.split("/").pop() || image.storage_path;
    if (image.storage_path.startsWith(localStoragePathPrefix)) {
      return storageName;
    }
  ...
  if (image.storagePath) {
    ...
    if (image.storagePath.startsWith(localStoragePathPrefix)) {
      return storageName;
    }
    return storageName.replace(/^\d+-/, "");
  }
  ...
  if (image.public_url) {
    return image.public_url.split("/").pop() || image.public_url;
  }
...
export async function uploadHostedImages(projectId, files) {
  const supabase = await getSupabaseServerClient();
  const uploaded = [];
  const sortedFiles = [...files].sort((a, b) => compareImageNames(a.name, b.name));
...
export async function updateHostedImageAlt(projectId, imageId, alt) {
  const supabase = await getSupabaseServerClient();
...
export async function deleteHostedImages(projectId, imageIds = []) {
  const supabase = await getSupabaseServerClient();
  const normalizedIds = [...new Set(imageIds.filter(Boolean))];
  ...
  if (!images || images.length !== normalizedIds.length) {
    throw new Error("部分图片不存在，请刷新后重试。");
  }
  ...
  const deletedUrls = new Set(images.map((image) => image.public_url));
  if (project?.cover_image_url && deletedUrls.has(project.cover_image_url)) {
    const { data: nextImage } = await supabase
      .from("project_images")
      .select("public_url")
      .eq("project_id", projectId)
    ...
    await supabase
      .from("projects")
      .update({ cover_image_url: nextImage?.public_url || null })
      .eq("id", projectId);
  }
...
export async function reorderHostedProjectImagesByFilename(projectId) {
  const supabase = await getSupabaseServerClient();
  const { data: images, error } = await supabase
    .from("project_images")
    .select("id, storage_path, alt, public_url")
    .eq("project_id", projectId);
  ...
  const sortedImages = [...(images || [])].sort((a, b) => {
    const nameCompare = compareImageNames(extractImageSortName(a), extractImageSortName(b));
    if (nameCompare !== 0) {
      return nameCompare;
    }
