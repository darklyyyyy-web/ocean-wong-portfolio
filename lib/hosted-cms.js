...
import { getLocalProjectBySlug, getLocalProjectImageFilePath, getLocalProjects } from "@/lib/projects";
...
export async function importLocalProjectToHosted(projectSlug) {
  ...
  if (existingImages?.length) {
    await supabase.storage.from(supabaseBucket).remove(
      existingImages
  ...
  for (const [index, image] of localProject.images.entries()) {
    const extension = path.extname(image.fileName || "").toLowerCase();
    const contentType = importMimeTypes[extension] || "application/octet-stream";
    const absolutePath = getLocalProjectImageFilePath(localProject.folderName, image.fileName);
    const fileBuffer = fs.readFileSync(absolutePath);
    const safeName = (image.fileName || `image-${index + 1}`).replace(/\s+/g, "-");
    const storagePath = `${projectId}/import-${index + 1}-${Date.now()}-${safeName}`;

    const { error: uploadError } = await supabase.storage.from(supabaseBucket).upload(storagePath, fileBuffer, {
      contentType,
      upsert: false
    });
    ...
    if (!coverSrc || image.fileName === localProject.cover) {
      coverSrc = publicUrl;
    }
  }
