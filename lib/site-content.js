...
const projectMetadataFile = path.join(process.cwd(), "data", "projects.json");

const defaultPages = {
  projectsTitle: "拍摄类型",
  projectsIntro: "先选择拍摄大类，再进入每个具体项目。项目文件夹仍然由本地后台自动识别。",
  categoryIntro: "先选择这个大类下的具体案例，再进入查看完整照片内容。",
  contactTitle: "联系我",
...
function getProjectDerivedCategories() {
  const metadata = readJson(projectMetadataFile) || {};
  const map = new Map();

  Object.values(metadata).forEach((project) => {
...
defaultContent.categories = mergeCategories([
  ...defaultContent.categories,
  ...getProjectDerivedCategories()
]);
